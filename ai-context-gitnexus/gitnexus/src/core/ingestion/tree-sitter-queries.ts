...
; Spring annotations: @RequestMapping("/path"), @GetMapping("/path"), etc.
; Full annotation with single positional string argument
(annotation
  name: (identifier) @decorator.name
  arguments: (annotation_argument_list
    (string_literal) @decorator.arg)) @decorator

; Full annotation with named argument: value = "/path"
(annotation
  name: (identifier) @decorator.name
  arguments: (annotation_argument_list
    (element_value_pair
      key: (identifier) @_key
      value: (string_literal) @decorator.arg))) @decorator

; Full annotation with named path argument: path = "/path"
(annotation
  name: (identifier) @decorator.name
  arguments: (annotation_argument_list
    (element_value_pair
...
; Marker annotation (no arguments): @RestController, @Controller, etc.
(marker_annotation
  name: (identifier) @decorator.name) @decorator.marker

; HTTP client calls: restTemplate.getForObject(url, ...), webClient.get().uri("/path")
(method_invocation
  object: (_) @http_client.receiver
  name: (identifier) @http_client.method
  arguments: (argument_list
    (string_literal) @http_client.url)) @http_client
`;
