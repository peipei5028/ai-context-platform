...
export interface WorkerExtractedData {
  ...
  /**
   * True when data comes from the sequential fallback path.
   * Imports, calls, heritage, etc. were already added to the graph directly
   * — only decoratorRoutes and fetchCalls need to be consumed by the caller.
   */
  sequential?: boolean;
  /**
...
const processParsingSequential = async (
  ...
  const total = files.length;
  const skippedLanguages = new Map<string, number>();
  const seqDecoratorRoutes: ExtractedDecoratorRoute[] = [];
  const seqFetchCalls: ExtractedFetchCall[] = [];
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    matches.forEach((match) => {
      ...
      // Collect decorator routes (Spring/NestJS/Flask annotations, Express routes)
      if (captureMap['decorator'] && captureMap['decorator.name']) {
        const decoratorName = captureMap['decorator.name'].text;
        const rawArg = captureMap['decorator.arg']?.text;
        const decoratorArg =
          rawArg && rawArg.length >= 2 && (rawArg.startsWith('"') || rawArg.startsWith("'"))
            ? rawArg.slice(1, -1)
            : rawArg;
        ...
        if (ROUTE_DECORATOR_NAMES.has(decoratorName)) {
          ...
          const httpMethod = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].includes(method)
            ? method
            : 'GET';
          seqDecoratorRoutes.push({
            filePath: file.path,
            routePath,
            httpMethod,
            decoratorName,
            lineNumber: captureMap['decorator'].startPosition.row + lineOffset,
          });
        }
        return;
      }

      // Skip marker annotations (no route path)
      if (captureMap['decorator.marker']) return;
  ...
  return {
    ...
    heritage: [],
    routes: [],
    fetchCalls: seqFetchCalls,
    decoratorRoutes: seqDecoratorRoutes,
    toolDefs: [],
    ormQueries: [],
    constructorBindings: [],
    fileScopeBindings: [],
...
export const processParsing = async (
  ...
  // Fallback: sequential parsing — now returns decorator/fetch data for route extraction.
  // The sequential path adds definition nodes, imports, calls, and heritage directly
  // to the graph and symbol table, so we only return the data that the sequential
  // path doesn't handle in-band (decorator routes and fetch calls).
