...
const processFileGroup = (
  ...
  for (const file of files) {
    ...
    for (const match of matches) {
      ...
      // Handles both TS/Python decorators and Java annotations
      if (captureMap['decorator'] && captureMap['decorator.name']) {
        const decoratorName = captureMap['decorator.name'].text;
        // Java string_literal includes quotes — strip them; TS/Python string_fragment does not
      ...
      // Java marker annotations (no arguments): @RestController, @Controller, etc.
      if (captureMap['decorator.marker'] && captureMap['decorator.name']) {
        const decoratorName = captureMap['decorator.name'].text;
        const decoratorNode = captureMap['decorator.marker'];
        // Store by the marker annotation's end line for association with definitions
        fileDecorators.set(decoratorNode.endPosition.row, {
          name: decoratorName,
        });
        continue;
      ...
      if (captureMap['http_client'] && captureMap['http_client.url']) {
        ...
        // Java string_literal includes quotes — strip them; TS/Python fragments do not
        const rawUrl = captureMap['http_client.url'].text;
        const url =
          rawUrl.length >= 2 && (rawUrl.startsWith('"') || rawUrl.startsWith("'"))
            ? rawUrl.slice(1, -1)
            : rawUrl;
