...
export async function runChunkedParseAndResolve(
  ...
  try {
    for (let chunkIdx = 0; chunkIdx < numChunks; chunkIdx++) {
      ...
      if (chunkWorkerData && !chunkWorkerData.sequential) {
        // Full worker data — imports, calls, heritage were extracted by workers
        ...
        // Sequential fallback or sequential-mode worker data
        await processImports(graph, chunkFiles, astCache, ctx, undefined, repoPath, allPaths);
        sequentialChunkPaths.push(chunkPaths);
        // Collect decorator routes and fetch calls from sequential path
        if (chunkWorkerData) {
          if (chunkWorkerData.decoratorRoutes?.length) {
            for (const item of chunkWorkerData.decoratorRoutes) allDecoratorRoutes.push(item);
          }
