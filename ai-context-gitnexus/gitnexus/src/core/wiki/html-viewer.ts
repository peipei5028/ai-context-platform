...
function buildHTML(
  ...
  parts.push('<div class="empty-state"><h2>加载中…</h2></div>');
  parts.push('</main>');
  parts.push('</div>');
...
(function() {
  ...
  function renderNav() {
    ...
    html += '<a class="nav-item overview" data-page="overview" href="#overview">总览</a>';
    html += '</div>';
    if (TREE.length > 0) {
  ...
  function navigateTo(page) {
    ...
    if (!md) {
      contentEl.innerHTML = '<div class="empty-state"><h2>页面未找到</h2><p>' + escH(page) + '.md 不存在。</p></div>';
      return;
    }
